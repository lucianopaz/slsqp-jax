...
from slsqp_jax.qp_solver import solve_qp, QPResult as QPSolverResult
...
class SLSQP(optx.AbstractMinimiser):
    ...
    def init(
        ...
        if self.eq_constraint_fn is not None and m_eq > 0:
            ...
            eq_jac = jax.jacfwd(lambda x: self.eq_constraint_fn(x, args))(y)
        else:
        ...
        if self.ineq_constraint_fn is not None and m_ineq > 0:
            ...
            ineq_jac = jax.jacfwd(lambda x: self.ineq_constraint_fn(x, args))(y)
        else:
    ...
    def step(
        ...
        if self.eq_constraint_fn is not None and m_eq > 0:
            eq_jac_new = jax.jacfwd(lambda x: self.eq_constraint_fn(x, args))(y_new)
        else:
            eq_jac_new = jnp.zeros((m_eq, n))
            
        if self.ineq_constraint_fn is not None and m_ineq > 0:
            ineq_jac_new = jax.jacfwd(lambda x: self.ineq_constraint_fn(x, args))(y_new)
        else:
