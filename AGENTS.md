...
where W = (γS, Y) and N is a small 2k×2k matrix. Powell's damping is applied to each curvature pair before storage to ensure positive definiteness.  The damping threshold is configurable via `damping_threshold` on `SLSQP` (default 0.2); setting it to 0.0 disables damping entirely for well-conditioned PD objectives.
...
4. **Hessian update**: Append new curvature pair (s, y) to L-BFGS history (or skip if using exact HVPs). The gradient difference `y_k = ∇_x L(x_{k+1}, λ_{k+1}) − ∇_x L(x_k, λ_{k+1})` uses the **same** (blended) multipliers at both points to satisfy the secant condition (Nocedal & Wright §18.3). The initial Hessian uses **Shanno-Phua per-variable diagonal scaling**: `B₀ = diag(d)` where `d_i = y_i² / (yᵀs)`, clipped to `[1e-2, 1e6]`. Unlike scalar `γI`, this captures per-variable curvature, critical for ill-conditioned problems.
