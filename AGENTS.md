...
- **`ProjectedCGCraig`**: Projected CG with CRAIG's method (Golub-Kahan bidiagonalization) replacing Cholesky. Each projection iteratively computes `x = Aᵀ(AAᵀ)⁻¹b` without forming `AAᵀ`, eliminating the O(m³) factorization and the ε regularisation. Better equality constraint satisfaction (no regularisation bias). Multiplier recovery via CG on normal equations `AAᵀ λ = A(Bd+g)`. Cost: O(mn · t_craig) per projection. Reference: Craig (1981); Golub & Kahan (1965).

- **`MinresQLPSolver`**: Solves the full (n+m)×(n+m) saddle-point KKT system `[B Aᵀ; A 0][d; λ] = [-g; b]` directly using Preconditioned MINRES-QLP (PMINRES-QLP, Table 3.5 of Choi 2006). No null-space projection or separate multiplier recovery needed — d and λ come out simultaneously. Handles indefinite/singular systems natively. All iterations use QLP mode (both left QR and right QLP orthogonalizations) with numerically stable SymOrtho rotations (Table 2.9). Uses a **block-diagonal SPD preconditioner** `M⁻¹ = diag(B_diag⁻¹, S⁻¹)` where `B_diag⁻¹` is the L-BFGS inverse Hessian diagonal and `S = A B_diag⁻¹ Aᵀ` is the Schur complement. This satisfies the SPD preconditioner requirement from Section 3.4 of Choi (2006). When the bound-fixing loop supplies a `free_mask`, the preconditioner's primal block is masked to the free subspace (`v → free * B⁻¹(free * v)`) to prevent L-BFGS cross-coupling from leaking into the zero dimensions of the reduced KKT system; without this masking the preconditioned Lanczos vectors become contaminated and constraint satisfaction degrades. Cost: O((n+m) · t_minres) per solve. Reference: Choi (2006), PhD thesis, Stanford; Choi, Paige & Saunders, SIAM J. Sci. Comput. 33(4), 2011.
