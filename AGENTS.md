...
**Preconditioning**: The CG solver supports an optional preconditioner M ≈ B̃⁻¹. By default, the L-BFGS inverse Hessian (two-loop recursion, Algorithm 7.4 of Nocedal & Wright) is used. When equality constraints are present, the preconditioner is upgraded to B̃⁻¹ via the Woodbury identity so it matches the actual QP system matrix B̃ = B + (1/μ) A_eq^T A_eq, where μ is the adaptive proximal parameter. For projected CG, the preconditioner is applied as z = P(M(P(r))) to keep the search direction in the constraint null space.
...
**Multiplier stability:** Wright (SIAM J. Optim., 2002, Theorem 5.3) proved that SQP superlinear convergence requires multiplier stability across iterations. The solver promotes this through: (1) warm-starting the QP active set from the previous iteration, (2) an adaptive EXPAND tolerance tied to the outer KKT residual, (3) adaptive proximal multiplier stabilization (sSQP, always active for equality-constrained problems, with `mu = max(kkt_residual^tau, mu_min)` per Wright eq 6.6; controlled by `proximal_tau` and `proximal_mu_min` on `SLSQP`), and (4) alpha-scaled multiplier blending (`lambda_new = lambda_prev + alpha * (lambda_QP - lambda_prev)`) for L-BFGS secant pair consistency when the line search accepts a partial step. Raw QP multipliers are stored in the state for convergence checking.
