...
- **`MinresQLPSolver`**: Solves the full (n+m)×(n+m) saddle-point KKT system `[B Aᵀ; A 0][d; λ] = [-g; b]` directly using MINRES-QLP. No null-space projection or separate multiplier recovery needed — d and λ come out simultaneously. Handles indefinite/singular systems natively. Optional Schur complement preconditioner `S = A G⁻¹ Aᵀ` where G = diag(B₀). Cost: O((n+m) · t_minres) per solve. Reference: Choi, Paige & Saunders, SIAM J. Sci. Comput. 33(4), 2011.
...
**Zero-step convergence detection:** When the QP solver repeatedly converges with `||d|| < atol`, the current point satisfies the QP's KKT conditions (within `cg_tol`) even though the outer stationarity criterion `||∇L|| <= rtol * max(|L|, 1)` may not be met — typically because of residual multiplier imprecision from the regularized Cholesky projection. After `zero_step_patience` (default 3) consecutive zero-step iterations, the solver declares `successful` convergence rather than waiting for the merit-based stagnation counter (which would report `nonlinear_divergence`). Controlled by `zero_step_patience` on `SLSQP`.
